...
Le serveur MCP expose **11 outils** (v1.5.2+). Six d'entre eux sont utilisables **sans TopSolid lancé** — uniquement pour consulter la base de connaissance :
...
| `topsolid_get_recipe(name)` | Code C# d'une des 118 recettes production |
| `topsolid_compile(code)` | Dry-run compile check sans exécuter |
| `topsolid_search_examples(query)` | Cherche dans 225+ snippets de corpora production |
| `topsolid_whats_new(version)` | Changelog de l'API par version TopSolid |

Les 5 autres outils (`execute_script`, `run_recipe`, `get_state`, `modify_script`, `topsolid_connect`) nécessitent TopSolid actif et servent au runtime agent.
